...
def run(spark: SparkSession, jdbc_url: str) -> list[Inquery]:
    try:
        ...
        # Fallback in case table names in publish_table.py are the ones that actually exist
        # or if there was a discrepancy.
        try:
            products_df = spark.read.jdbc(url=jdbc_url, table="publish_products")
            # publish_orders doesn't have an obvious alternative in publish_table.py DDL
            # as it seems to be a join of header and detail.
        except:
